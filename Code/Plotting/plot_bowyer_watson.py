...
def nice_plots():
    ...
    wh_vtx = np.empty(3, dtype=np.int64)
    ### _walk
    walk_pts = np.empty(6, dtype=np.float64)
    walk_nbr = np.empty(3, dtype=np.int64)
    ### _identify_cavity
    ...
    ### _cavity_helper
    ch_pts = np.empty(6, dtype=np.float64)
    ch_vtx = np.empty(3, dtype=np.int64)
    ### _make_Delaunay_ball
    mDb_new_tri_indices = np.empty(50, dtype=np.int64)
    mDb_new_tri_vtx = np.empty(3*50, dtype=np.int64)
    mDb_new_tri_nbr = np.empty(3*50, dtype=np.int64)
    ...
    vertices_ID = DT.vertices_ID.copy()
    neighbour_ID = DT.neighbour_ID.copy()
    num_tri_v = DT.num_tri_v
    num_tri_n = DT.num_tri_n
