...
def _walk(
    ...
    if store == True:
        all_tri_crossed = []
        all_tri_crossed.append(t_index)
    while True:
        ...
        if store == True:
            all_tri_crossed.append(wh_t_op_index)
    ...
    if store == False:
        ...
        return t_index, all_tri_crossed
...
def add_point(
    ...
    if point_already_exists == False:
        enclosing_tri, all_tri_crossed = _walk(
            points[2*point_id],   # point_x
            points[2*point_id+1], # point_y
            old_tri,              # t_index
            vertices_ID,          # vertices_ID
        ...
        cx_all = []
        cy_all = []
        for i in range(len(all_tri_crossed)):
            t = all_tri_crossed[i]
            vtx = vertices_ID[3*t:3*t+3]
            idx = np.where(vtx!=-1)[0]
            if len(idx) == 3:
                plt.fill(points[2*vtx], points[2*vtx+1], alpha=0.5, facecolor='C1')
                cx = np.sum(points[2*vtx])/3
                cy = np.sum(points[2*vtx+1])/3
                if len(cx_all) == 0:
                    plt.plot(cx, cy, 'v', color='brown')
                else:
                    plt.plot(cx, cy, 'o', color='brown')
                if len(cx_all) >= 1:
                    plt.plot([cx_all[-1], cx], [cy_all[-1], cy], '-', linewidth=0.8, color='brown')
                cx_all.append(cx)
                cy_all.append(cy)
        plt.plot(points[2*point_id], points[2*point_id+1], 'x', color='k', mew=10, ms=3)
        plt.xticks([])
        plt.yticks([])
        plt.axis('equal')
        plt.savefig('bowyer_watson_walk.png', dpi=300, bbox_inches='tight')
        # plt.show()
        ...
        for i in range(ic_bad_tri_end):
            ...
            idx = np.where(vtx!=-1)[0]
            if len(idx) == 3:
                plt.fill(points[2*vtx], points[2*vtx+1], alpha=0.5, facecolor='C3')
                # cx = np.sum(points[2*vtx])/3
                # cy = np.sum(points[2*vtx+1])/3
                # if len(cx_all) == 0:
                #     plt.plot(cx, cy, 'v', color='brown')
                ...
                # cx_all.append(cx)
                # cy_all.append(cy)
        for i in range(ic_boundary_tri_end):
            t = ic_boundary_tri[i]//3
            vtx = vertices_ID[3*t:3*t+3]
            idx = np.where(vtx!=-1)[0]
            if len(idx) == 3:
                plt.fill(points[2*vtx], points[2*vtx+1], alpha=0.5, facecolor='C2')
        plt.plot(points[2*point_id], points[2*point_id+1], 'x', color='k', mew=10, ms=3)
        plt.xticks([])
        plt.yticks([])
        plt.axis('equal')
        plt.savefig("bowyer_watson_cavity.png", dpi=300, bbox_inches='tight')
        # plt.show()
        ...
        plt.clf()
        DT.points = points.copy()
        DT.vertices_ID = vertices_ID.copy()
        DT.neighbour_ID = neighbour_ID.copy()
        DT.num_tri_n = num_tri_n
        DT.num_tri_v = num_tri_v
        DT.plotDT()
        for i in range(mDb_new_tri_indices_end):
            t = mDb_new_tri_indices[i]
            vtx = vertices_ID[3*t:3*t+3]
            idx = np.where(vtx!=-1)[0]
            if len(idx) == 3:
                plt.fill(points[2*vtx], points[2*vtx+1], alpha=0.5, facecolor='C4')
                # cx = np.sum(points[2*vtx])/3
                # cy = np.sum(points[2*vtx+1])/3
                # if len(cx_all) == 0:
                #     plt.plot(cx, cy, 'v', color='brown')
        ...
        plt.yticks([])
        plt.axis('equal')
        plt.savefig("bowyer_watson_dball.png", dpi=300, bbox_inches='tight')
        # plt.show()
...
class Delaunay2D:
    ...
    def exportDT(self):
        ...
        g_vertices = np.array([], dtype=np.int64)
        for i in np.arange(0, self.num_tri_v):
            ...
            if len(idx) == 0:
                ...
                if idx == 0:
                    ...
                    a_idx = 0
                    b_idx = 1
                ax = self.points[2*self.vertices_ID[3*i+a_idx]]
                ay = self.points[2*self.vertices_ID[3*i+a_idx]+1]
                bx = self.points[2*self.vertices_ID[3*i+b_idx]]
                by = self.points[2*self.vertices_ID[3*i+b_idx]+1]
                nbr = self.neighbour_ID[3*i+idx]//3
                point_in_nbr = self.neighbour_ID[3*i+idx]%3
                point_id = self.vertices_ID[3*nbr+point_in_nbr]
                px = self.points[2*point_id]
                py = self.points[2*point_id+1]
                cx = 0.5*(ax+bx)
                cy = 0.5*(ay+by)
                d = ((cx-px)**2+(cy-py)**2)**0.5
                p2x = ((d+0.2)*cx-0.2*px)/d
                p2y = ((d+0.2)*cy-0.2*py)/d
                points = np.append(points, [p2x, p2y])
                g_vertices = np.append(g_vertices, [self.vertices_ID[3*i+a_idx], self.vertices_ID[3*i+b_idx], int(0.5*len(points)-1)])
        return points, vertices, g_vertices
    ...
    def plotDT(self):
        ...
        import matplotlib.pyplot as plt
        points, vertices, g_vertices = self.exportDT()
        ...
        plt.triplot(
            points[0::2],
            points[1::2],
            g_vertices.reshape(int(len(g_vertices)/3), 3),
            '--',
            linewidth=0.75
        )
        plt.triplot(
...
def nice_plots():
    ...
    for i in range(5):
        for j in range(5):
            points.append([i*0.2, j*0.2])

    points = np.array(points).reshape(2*len(points))
    points = points + 0.1*np.random.rand(len(points))
